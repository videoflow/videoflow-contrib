...
Unlike the ML solutions, this also works on a host without any
`videoflow_contrib` packages installed — the graph only needs core videoflow.
For a smoke run that ends on its own, set `duration_s: 20` in `config.yaml`;
with `0` the flow runs until Ctrl-C. Manual variant:

```bash
cd /path/to/videoflow && docker compose up -d nats redis   # NATS :4222, Redis :6379

cd /path/to/videoflow-contrib/solutions/toy_fusion
