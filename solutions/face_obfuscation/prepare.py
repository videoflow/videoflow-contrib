...
detector weights into the videoflow cache, so the first worker doesn't stall on
a download (and so an offline or air-gapped run works at all).

`videoflow deploy face_obfuscation.py` and `videoflow run-local` run this
automatically inside the solution image before compiling; it can also be run by
hand:

    python prepare.py --config config.yaml [--force]

Idempotent: `get_file` skips anything already in ~/.videoflow/models (or in
work_dir, for the sample clip).
'''
...
def main():
    ap = argparse.ArgumentParser(description='Fetch the input video and pre-fetch the face-detector weights.')
