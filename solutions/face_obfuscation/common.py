...
``input_video`` may be left empty to use the bundled sample clip, which
``prepare.py`` downloads into ``work_dir`` and the graph reads from there.
'''
...
class Config:
    ...
    def resolve_input(self) -> str:
        ...
        stdout). ``prepare.py`` calls ``fetch_input`` before the workers run.
        '''
        if self.input_video:
